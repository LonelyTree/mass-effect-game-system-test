...
export default class BaseRestDialog extends Dialog5e {
  ...
  get promptNewDay() {
    const duration = CONFIG.DND5E.restTypes[this.config.type]
      ?.duration?.[game.settings.get(game?.system?.id ?? "massEffect", "restVariant")] ?? 0;
  ...
  async _prepareContext(options) {
    const context = {
      ...
      variant: game.settings.get(game?.system?.id ?? "massEffect", "restVariant")
    };
    ...
    if ( this.isPartyGroup ) {
      const restSettings = this.actor.getFlag(game?.system?.id ?? "massEffect", "restSettings") ?? {};
      context.request = [
        {
          field: new BooleanField({
  ...
  static async #handleFormSubmission(event, form, formData) {
    ...
    if ( this.isPartyGroup ) {
      ...
      this.actor.setFlag(game?.system?.id ?? "massEffect", "restSettings", data);
    }
