...
export default class ModuleArt {
  ...
  async #parseArtMapping(moduleId, mapping, credit) {
    let settings = game.settings.get(game?.system?.id ?? "massEffect", "moduleArtConfiguration")?.[moduleId];
  ...
  static getArtModules() {
    const settings = game.settings.get(game?.system?.id ?? "massEffect", "moduleArtConfiguration");
    const unsorted = [];
    const configs = [];
