...
export default class EnchantmentApplicationElement extends HTMLElement {
  ...
  connectedCallback() {
    ...
    const scaling = this.chatMessage.getFlag(game?.system?.id ?? "massEffect", "scaling");
    if ( scaling ) item = item.clone({ `flags.${game.system.id}.': scaling });
  ...
  async _onDrop(event) {
    ...
    const concentrationId = this.chatMessage.getFlag(game?.system?.id ?? "massEffect", "use.concentrationId");
    ...
    this.enchantmentActivity.applyEnchantment(
      this.chatMessage.getFlag(game?.system?.id ?? "massEffect", "use.enchantmentProfile"),
      droppedItem,
