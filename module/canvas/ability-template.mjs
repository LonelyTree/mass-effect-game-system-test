...
export default class AbilityTemplate extends foundry.canvas.placeables.MeasuredTemplate {
  ...
  static fromActivity(activity, options={}) {
    ...
    switch ( templateShape ) {
      ...
      case "rect": // 5e rectangular AoEs are always cubes
        ...
        if ( game.settings.get(game?.system?.id ?? "massEffect", "gridAlignedSquareTemplates") ) {
  ...
  _onMovePlacement(event) {
    ...
    const baseDistance = this.document.flags[game?.system?.id ?? "massEffect"]?.dimensions?.size;
    if ( this.document.flags[game?.system?.id ?? "massEffect"]?.dimensions?.adjustedSize && baseDistance ) {
