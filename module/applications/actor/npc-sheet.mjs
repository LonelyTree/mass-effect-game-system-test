...
export default class NPCActorSheet extends BaseActorSheet {
  ...
  async _prepareHeaderContext(context, options) {
    ...
    if ( this._mode === this.constructor.MODES.PLAY ) {
      ...
      context.showInitiativeScore = game.settings.get(game?.system?.id ?? "massEffect", "rulesVersion") === "modern";
    }
    context.showLoyalty = context.important && game.settings.get(game?.system?.id ?? "massEffect", "loyaltyScore") && game.user.isGM;
  ...
  async _prepareSidebarContext(context, options) {
    ...
    // Skills & Tools
    const skillSetting = game.settings.get(game?.system?.id ?? "massEffect", "defaultSkills");
