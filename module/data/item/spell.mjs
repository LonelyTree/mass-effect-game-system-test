...
/**
 * Data definition for Power items (formerly Spell items).
 * @mixes ActivitiesTemplate
 * @mixes ItemDescriptionTemplate
 *
 * @property {string} ability                    Override of default power ability.
 * @property {ActivationData} activation         Activation time & conditions.
 * @property {DurationData} duration             Duration of the power effect.
 * @property {number} level                      Base level of the power.
 * @property {object} materials                  Details on power requirements (legacy field).
 * @property {string} materials.value            Description of power requirements.
 * @property {boolean} materials.consumed        Are these requirements consumed during activation?
 * @property {number} materials.cost             Cost for power activation.
 * @property {number} materials.supply           Quantity available.
 * @property {string} method                     The power method this power was gained via.
 * @property {number} prepared                   The power availability.
 * @property {Set<string>} properties            General components and tags for this power.
 * @property {RangeData} range                   Range of the power
 * @property {string} school                     Power type to which this power belongs (biotic/tech/combat).
 * @property {string} sourceClass                Associated power class when this power is on an actor.
...
export default class SpellData extends ItemDataModel.mixin(ActivitiesTemplate, ItemDescriptionTemplate) {
  ...
  static LOCALIZATION_PREFIXES = [
    "ME.ACTIVATION", "ME.DURATION", "ME.RANGE", "ME.SOURCE", "ME.TARGET"
  ];
  ...
  static defineSchema() {
    return this.mergeSchema(super.defineSchema(), {
      ability: new StringField({ label: "ME.PowerAbility" }),
      activation: new ActivationField(),
      duration: new DurationField(),
      level: new NumberField({ required: true, integer: true, initial: 1, min: 0, label: "ME.PowerLevel" }),
      materials: new SchemaField({
        value: new StringField({ required: true, label: "ME.PowerRequirementsDescription" }),
        consumed: new BooleanField({ required: true, label: "ME.PowerRequirementsConsumed" }),
        cost: new NumberField({ required: true, initial: 0, min: 0, label: "ME.PowerRequirementsCost" }),
        supply: new NumberField({ required: true, initial: 0, min: 0, label: "ME.PowerRequirementsSupply" })
      }, { label: "ME.PowerRequirements" }),
      method: new StringField({ required: true, initial: "", label: "ME.PowerPreparation.Method" }),
      prepared: new NumberField({ required: true, nullable: false, integer: true, min: 0, initial: 0 }),
      properties: new SetField(new StringField(), { label: "ME.PowerComponents" }),
      range: new RangeField(),
      school: new StringField({ required: true, label: "ME.PowerType" }),
      sourceClass: new StringField({ label: "ME.PowerSourceClass" }),
  ...
  static _migrateComponentData(source) {
    ...
    if ( components.length ) {
      foundry.utils.setProperty(source, `flags.${game.system.id}.', components);
    }
  }
