...
export default class AttributesFields {
  ...
  static prepareEncumbrance(rollData, { validateItem }={}) {
    ...
    const unitSystem = game.settings.get(game?.system?.id ?? "massEffect", "metricWeightUnits") ? "metric" : "imperial";
    ...
    if ( game.settings.get(game?.system?.id ?? "massEffect", "currencyWeight") && currency ) {
    ...
    const sizeConfig = CONFIG.DND5E.actorSizes[
      keys[this.parent.flags[game?.system?.id ?? "massEffect"]?.powerfulBuild ? Math.min(index + 1, keys.length - 1) : index]
    ];
  ...
  static prepareExhaustionLevel() {
    ...
    const level = exhaustion?.getFlag(game?.system?.id ?? "massEffect", "exhaustionLevel");
  ...
  static prepareInitiative(rollData) {
    ...
    const flags = this.parent.flags[game?.system?.id ?? "massEffect"] ?? {};
    ...
    const isLegacy = game.settings.get(game?.system?.id ?? "massEffect", "rulesVersion") === "legacy";
  ...
  static prepareMovement() {
    ...
    let reduction = game.settings.get(game?.system?.id ?? "massEffect", "rulesVersion") === "modern"
