...
export default class SourceField extends SchemaField {
  constructor(fields={}, options={}) {
    fields = {
      ...
      rules: new StringField({
        initial: () => game.settings.get(game?.system?.id ?? "massEffect", "rulesVersion") === "modern" ? "2024" : "2014"
      }),
      ...fields
    };
  ...
  static prepareData(uuid) {
    ...
    this.bookPlaceholder = collection?.metadata?.flags?.[game?.system?.id ?? "massEffect"]?.sourceBook ?? SourceField.getModuleBook(pkg) ?? "";
  ...
  static getModuleBook(pkg) {
    if ( !pkg ) return null;
    const sourceBooks = pkg.flags?.[game?.system?.id ?? "massEffect"]?.sourceBooks;
