...
export default class CurrencyTemplate extends SystemDataModel {
   ...
   */
  get currencyWeight() {
    if ( !game.settings.get(game?.system?.id ?? "massEffect", "currencyWeight") ) return 0;
    const count = Object.values(this.currency).reduce((count, value) => count + value, 0);
    const currencyPerWeight = game.settings.get(game?.system?.id ?? "massEffect", "metricWeightUnits")
