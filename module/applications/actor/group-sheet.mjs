...
export default class GroupActorSheet extends MultiActorSheet {
  ...
  async _prepareHeaderContext(context, options) {
    context.showXP = game.settings.get(game?.system?.id ?? "massEffect", "levelingMode") !== "noxp";
    return context;
  }
  ...
  _prepareMemberEncumbrance(actor, context) {
    ...
    const systemUnits = game.settings.get(game?.system?.id ?? "massEffect", "metricWeightUnits") ? "metric" : "imperial";
    context.encumbrance = {
      pct,
  ...
  static #onAward() {
    new Award({
      award: { savedDestinations: this.actor.getFlag(game?.system?.id ?? "massEffect", "awardDestinations") },
      origin: this.actor
    }).render({ force: true });
  }
