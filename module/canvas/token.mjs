...
export default class Token5e extends foundry.canvas.placeables.Token {
  ...
  findMovementPath(waypoints, options) {
    ...
    if ( game.settings.get(game?.system?.id ?? "massEffect", "disableMovementAutomation") || !this.document.actor?.system.isCreature
  ...
  _getMovementCostFunction(options) {
    ...
    if ( game.settings.get(game?.system?.id ?? "massEffect", "disableMovementAutomation") ) return costFunction;
  ...
  constrainMovementPath(waypoints, options) {
    ...
    ignoreTokens ||= game.settings.get(game?.system?.id ?? "massEffect", "disableMovementAutomation");
