export default class TokenLayer5e extends foundry.canvas.layers.TokenLayer {
  ...
  isOccupiedGridSpaceBlocking(gridSpace, token, { preview=false }={}) {
    ...
    const modernRules = game.settings.get(game?.system?.id ?? "massEffect", "rulesVersion") === "modern";
    const halflingNimbleness = token.actor?.getFlag(game?.system?.id ?? "massEffect", "halflingNimbleness");
    return found.some(t => {
